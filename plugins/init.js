...
export default async function (
  { $http, route, $tools, $storeino, store, app },
  inject
) {
  if (process.server) {
    // response
    ...
    try {
      if (req.body && req.body.preview) {
        ...
        const body = {
          data: JSON.parse(req.body.preview.data),
          schema: JSON.parse(req.body.preview.schema),
        };
    ...
    const STOREINO_CART = cookies["STOREINO-CART"]
      ? cookies["STOREINO-CART"]
      : "[]";
    store.state.cart = JSON.parse(STOREINO_CART);

    // init Wishlist
    const STOREINO_WISHLIST = cookies["STOREINO-WISHLIST"]
      ? cookies["STOREINO-WISHLIST"]
      : "[]";
    ...
    if (!store.state.currency.code) {
      const { code, symbol } =
        settings.store_currencies.find((c) => c.default) ||
        settings.store_currencies[0];
      store.state.currency = { code, symbol };
    } else if (
      settings.store_currencies.find((c) => c.code == store.state.currency.code)
    )
      store.state.currency.symbol = settings.store_currencies.find(
        (c) => c.code == store.state.currency.code
      ).symbol;
    if (!store.state.language.code) {
      const { code, name } =
        settings.store_languages.find((c) => c.default) ||
        settings.store_languages[0];
    ...
    try {
      ...
      const url =
        store.state.baseURL != "https://api-stores.storeino.com/api"
          ? "https://appstatic.storeino.world"
          : "https://appstatic.storeino.com";
      const { data: objects } = await $http.get(`${url}/all/store`, {
        params: { names },
      });
    ...
    StoreinoApp.$store = {
      ...
      customUpdate: async function (module, params, data, headers = {}) {
        let response = await $http.post(`/${module}/me`, data, {
          params,
          headers,
        });
        return response.data;
    ...
    if (
      ...
      if (route.query.fbclid) {
        localStorage.setItem(
          "__fbc",
          `fb.1.${Date.now()}.${route.query.fbclid}`
        );
      }
    ...
    if (settings.snapchat_pixel && settings.snapchat_pixel.length > 0) {
      ...
      (function (e, t, n, tr) {
        ...
        a.queue = [];
        e.snapPixel = function (id, email = "") {
          snaptr("init", id, { user_email: email });
        };
        e.snapPageView = function (d = {}) {
          snaptr(tr, "PAGE_VIEW", d);
        };
        e.snapPurchase = function (d = {}) {
          snaptr(tr, "PURCHASE", d);
        };
        e.snapViewContent = function (d = {}) {
          snaptr(tr, "VIEW_CONTENT", d);
        };
        e.snapAddToCart = function (d = {}) {
          snaptr(tr, "ADD_CART", d);
        };
        e.snapAddToWishlist = function (d = {}) {
          snaptr(tr, "ADD_WISHLIST", d);
        };
        e.snapSignUp = function (d = {}) {
          snaptr(tr, "SIGN_UP", d);
        };
        var s = "script";
    ...
    // Twitter Pixel
    if (
      !app.context.store.state.isPreview &&
      settings &&
      ...
      !(function (e, t, n, s, u, a) {
        ...
        e.twitterPixel = function (id) {
          twq("init", id);
        };
        e.twitterPageView = function (d = {}) {
          twq("track", "PageView");
        };
        e.twitterViewContent = function (d = {}) {
          twq("track", "ViewContent", d);
        };
        e.twitterAddToCart = function (d = {}) {
          twq("track", "AddToCart", d);
        };
        e.twitterAddToWishlist = function (d = {}) {
          twq("track", "AddToWishlist", d);
        };
        e.twitterInitiateCheckout = function (d = {}) {
          twq("track", "InitiateCheckout", d);
        };
        e.twitterSearch = function (d = {}) {
          twq("track", "Search", d);
        };
        e.twitterAddPaymentInfo = function (d = {}) {
          twq("track", "AddPaymentInfo", d);
        };
        e.twitterSignup = function (d = {}) {
          twq("track", "Signup", d);
        };
        e.twitterCompleteRegistration = function (d = {}) {
          twq("track", "CompleteRegistration", d);
        };
        e.twitterDownload = function (d = {}) {
          twq("track", "Download", d);
        };
        e.twitterPurchase = function (d = {}) {
          twq("track", "Purchase", d);
        };
      ...
      if (route.query.pixel) {
        ...
        twitterPurchase({
          content_ids: pixel.contents.map((p) => {
            return p.id;
          }),
    ...
    if (settings.tiktok_pixel && settings.tiktok_pixel.length > 0) {
      ...
      !(function (w, d, t) {
        ...
        w.tiktokPixel = function (id) {
          ttq.load(id);
        };
        w.tiktokPageView = function () {
          ttq.page();
        };
        w.tiktokAddPaymentInfo = function (s = {}) {
          ttq.track("AddPaymentInfo", s);
        };
        w.tiktokAddToCart = function (s = {}) {
          ttq.track("AddToCart", s);
        };
        w.tiktokAddToWishlist = function (s = {}) {
          ttq.track("AddToWishlist", s);
        };
        w.tiktokClickButton = function (s = {}) {
          ttq.track("ClickButton", s);
        };
        w.tiktokPurchase = function (s = {}) {
          ttq.track("CompletePayment", s);
        };
        w.tiktokCompleteRegistration = function (s = {}) {
          ttq.track("CompleteRegistration", s);
        };
        w.tiktokViewContent = function (s = {}) {
          ttq.track("ViewContent", s);
        };
        w.tiktokSubscribe = function (s = {}) {
          ttq.track("Subscribe", s);
        };
        w.tiktokSubmitForm = function (s = {}) {
          ttq.track("SubmitForm", s);
        };
        w.tiktokSearch = function (s = {}) {
          ttq.track("Search", s);
        };
        w.tiktokPlaceAnOrder = function (s = {}) {
          ttq.track("PlaceAnOrder", s);
        };
        w.tiktokInitiateCheckout = function (s = {}) {
          ttq.track("InitiateCheckout", s);
        };
        w.tiktokDownload = function (s = {}) {
          ttq.track("Download", s);
        };
        w.tiktokContact = function (s = {}) {
          ttq.track("Contact", s);
        };
      })(window, document, "ttq");
      // Set Tiktok Pixel
    ...
    if (
      ...
      window.googleAdsEvent = (eventName) => {
        const eventsGroup = settings.google_ads.events.filter(
          (e) => e.name == eventName
        );
    ...
    if (
      store.state.settings.store_currencies &&
      store.state.settings.store_currencies.length > 1 &&
      store.state.IP
    ) {
      (async () => {
        try {
          const res = await $http.get(
            `https://api-views.storeino.com/api/geoLite/get?ip=${store.state.IP}`
          );
          if (
            res.data.geoplugin_currencyCode != store.state.currency.code &&
            store.state.settings.store_currencies.find(
              (c) => c.code == res.data.geoplugin_currencyCode
            )
          ) {
