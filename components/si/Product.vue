    ...
    <div class="relative flex flex-col wishlist-box">
        ...
        <div :class="page == 'wishlist' ? '' : 'wishlist opacity-0'"
            ...
            <transition name="fade">
                <button aria-label="wishlixt button" v-if="$store.state.wishlist.find(i => i._id == item._id)"
                    @click="removeFromWishlist" title="Wishlist" class="absolute top-0 right-0 z-10 m-4">
                    <svg aria-label="wishlixt button" class="w-5 h-5 translate text-primary" aria-hidden="true"
            ...
            <transition name="fade">
                <button aria-label="wishlixt button" v-if="!$store.state.wishlist.find(i => i._id == item._id)"
                    ...
                    <svg aria-label="wishlixt button" class="w-5 h-5 text-white translate" aria-hidden="true"
                        focusable="false" data-prefix="far" data-icon="heart" role="img"
                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        <path fill="currentColor"
        ...
        <div class="relative flex h-full ">
            <div class="flex flex-col w-full h-full">
                ...
                <div class="relative overflow-hidden image_box zoom"
                    ...
                    @mouseover="activeId = activeId = item._id" @mouseleave="activeId = activeId = null">
