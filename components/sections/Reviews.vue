<template>
    <div v-if="item && item.review.reviews.length > 0">
        ...
        <div class="mx-5 reviews  pb-7" id="reviews">
            <div class="flex flex-wrap ">
                <div v-for="(review, index) in reviews.results" :key="index" class="w-full mb-4">
                    <div class="flex flex-col justify-between h-full">
                        <div class="flex flex-col">
                            <div class="flex mb-3">
                                <si-avatar :name="`${review.customer.firstname} `"></si-avatar>
                                <span class="m-1"></span>
                                <span class="bg-text-primary" v-for="(star, i) in 5"
                                    :class="star <= review.rating ? ' text-yellow-500 ' : 'text-gray-400'" :key="i">
                                    <svg aria-label="star icon" class="w-3 h-3" width="20" height="20"
                            ...
                            <div class="flex items-start mb-3">
                                ...
                                <div>
                                    <b class="capitalize ml-font-bold text-sml">
                                        {{ `${review.customer.firstname} ${review.customer.lastname}` }}</b>
                                </div>
                            </div>
