                ...
                <transition name="header-slide">
                    <header>
                        <div id="header" class="absolute top-0 left-0 right-0 z-20 header header-color">
                            <div style="height: 3.8rem;" class="relative flex flex-wrap items-center justify-between px-5 lg:px-10">
                                <!-- NavBar -->
                                <nav class="hidden lg:block lg:w-2/5 header-text">
                                    <div v-if="menu" class="items-center hidden lg:flex text-sml scroll">
                                        <ul  v-for="(item, i) in menu.items" :key="i" @mouseover="activeId = activeId = item._id" @mouseleave="activeId = activeId = null">
                                            ...
                                            <li class="flex items-center py-5 chivron-box ml-mr-2">
                                                ...
                                                <button aria-label="chivron-down" class="opacity-0 chivron-down" v-if="item.childrens.length > 0" @click="activeId = activeId != item._id ? item._id : null">
                                            ...
                                            <transition name="slide">
                                                <div v-if="item._id == activeId && item.childrens.length > 0" class="absolute left-0 right-0 z-20 px-10 border-t border-b border-gray-300 header-color top-full">
                                                    <div class="flex items-center scroll">
                                                        <div v-for="(item,i) in item.childrens" :key="i" @mouseover="subItems = subItems = item.collectionId" @mouseleave="subItems = subItems = null">
                                                            <div class="flex items-center py-5 chivron-box ml-mr-2">
                                                                ...
                                                                <button aria-label="chivron-down" class="opacity-0 chivron-down" v-if="item.childrens.length > 0" @click="subItems = subItems != item.collectionId ? item.collectionId : null">
                                                            ...
                                                            <transition name="slide">
                                                                <div v-if="item.collectionId == subItems" class="absolute left-0 right-0 z-20 px-10 border-t border-gray-300 header-color top-full">
                                ...
                                <div class="w-auto lg:w-1/5">
                                    <div class="">
                                        <router-link class="" to="/">
                                            <si-image width="70" height="50" property="height" class="object-contain w-full h-10 md:h-12 lg:h-14" :src="section.logo ? section.logo.src : $store.state.defaults.logo" alt="Store logo"/>
                                        </router-link>
                                    </div>
                                </div>
                                <!-- logo -->
                                ...
                                <div class="w-auto lg:w-2/5 header-text" >
                                    <div class="flex items-center justify-end">
                                        <div class="flex items-center justify-between gap-2">
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <transition name="slide-right">
                                                    <div v-if="showSearch" @mouseleave="showSearch=false">
                                                        <form @submit.prevent="search" class="flex items-center py-1 border-b search border-primary" action="/shop?">
                                            ...
                                            <!-- search -->
                                            <!-- acount -->
                                            <div v-if="$settings.sections.header.icons.account" class="transition-all ease-in-out delay-150 user">
                                            ...
                                            <!-- cart -->
                                            <!-- show menu botton  -->
                                            <div class="block transition-all ease-in-out delay-150 lg:hidden">
                        ...
                        <div v-if="$settings.sections.header.icons.search" class="px-5 pb-5 lg:hidden header-color header-text">
                            <form @submit.prevent="search" class="flex items-center justify-between pb-2 border-b search border-primary" action="/shop?">
                                <input v-model="q" class="w-full bg-transparent outline-none text-sml" :placeholder="$settings.sections.header.search_text" type="search" name="q">
                                <button aria-label="Search button">
                                    <svg aria-label="search icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M17.17 16.48L12 11.36a5.5 5.5 0 10-4.22 2 5.41 5.41 0 003.51-1.27l5.14 5.13a.51.51 0 00.7 0 .5.5 0 00.04-.74zm-9.35-4.15a4.5 4.5 0 110-9 4.5 4.5 0 010 9z" fill="currentColor"></path></svg>
                                </button>
                            </form>
