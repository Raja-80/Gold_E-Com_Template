<template>
    <div class="px-12 lg:py-32 py-16">
        ...
        <div v-if="!loading && items.length > 0 && posts_info.show_title" class="flex flex-col justify-center items-center lg:pb-24 pb-14 px-5 ">

            <h1 class="text-black font-semibold lg:text-3xl md:text-2xl sm:text-xl text-xl text-center ">
        ...
        <div>
            ...
            <carousel v-if=" !loading && items && items.length > 0 " 
                :perPageCustom="[[240, 1], [595, 2], [768, 3], [1024, 4]]"
                :autoplay="true"
                ...
                <slide v-for="(item, index) in items" :key="index" class="px-1">
            ...
            <div v-if="!loading && items.length==0" class="px-5 my-5 xl:my-10 xl:px-10">
                <div class="flex flex-wrap items-center p-5 bg-white border border-gray-300 rounded-lg">
                        ...
                        <div class="flex items-center justify-center w-full mt-8 lg:w-1/2 align-center lg:mt-0 lg:items-start lg:justify-start ">
                            <div>
                                <h2 v-if="this.$store.state.language.code == 'AR'" class="text-lg font-bold lg:text-xl text-center-res">{{ 'ليس لديك أي منشورات' }}</h2>
                                <h2 v-else-if="this.$store.state.language.code == 'FR'" class="text-lg font-bold lg:text-xl text-center-res">{{ 'Vous n\'avez aucune posts' }}</h2>
                                <h2 v-else class="text-lg font-bold lg:text-xl text-center-res">{{ 'You don\'t have any posts' }}</h2>
                                <p v-if="this.$store.state.language.code == 'AR'" class="mt-2 text-sml text-center-res">{{ 'انتقل إلى مسؤول المتجر الخاص بك > المتجر > المدونة' }} <a href="https://business.storeino.com/stores/posts" target="_blank" class="underline text-primary primary-hover">{{ 'اضف جديد' }}</a></p>
                                <p v-else-if="this.$store.state.language.code == 'FR'" class="mt-2 text-sml text-center-res">{{ 'Accédez à l\'administration de votre Boutique > Blog ' }} <a href="https://business.storeino.com/stores/posts" target="_blank" class="underline text-primary primary-hover">{{ 'Ajouter nouveau' }}</a></p>
                                <p v-else class="mt-2 text-sml text-center-res">{{ 'Go to your store admin > store > Blog ' }} <a href="https://business.storeino.com/stores/posts" target="_blank" class="underline text-primary primary-hover">{{ 'Add new' }}</a></p>
                        </div>
                    </div>
                </div>
            </div>
