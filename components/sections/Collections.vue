<template>
    <div>
        ...
        <div v-if="!loading && items.length==0" class="my-5 xl:my-10 px-5 xl:px-10">
            <div class="flex flex-wrap items-center bg-white p-5 border border-gray-300 rounded-lg">
            <div class="w-full lg:w-1/2 flex items-center justify-center lg:items-start lg:justify-start ">
            ...
            <div class="w-full lg:w-1/2 align-center mt-8 lg:mt-0 flex items-center justify-center lg:items-start lg:justify-start ">
                <div>
                    ...
                    <p v-if="this.$store.state.language.code == 'AR'" class="text-sml mt-2 text-center-res">{{ 'انتقل إلى مسؤول المتجر الخاص بك > المنتجات > المجموعات' }} <a href="//business.storeino.com/stores/collections" target="_blank" class="text-primary underline text-hover">{{ 'اضف جديد' }}</a></p>
                    <p v-else-if="this.$store.state.language.code == 'FR'" class="text-sml mt-2 text-center-res">{{ 'Accédez à l\'administration de votre boutique > produits > collections >' }} <a href="//business.storeino.com/stores/collections" target="_blank" class="text-primary underline text-hover">{{ 'Ajouter nouveau' }}</a></p>
                    <p v-else class="text-sml mt-2 text-center-res">{{ 'Go to your store admin > products > collections >' }} <a href="//business.storeino.com/stores/collections" target="_blank" class="text-primary underline text-hover">{{ 'Add new' }}</a></p>
                    <div class="flex justify-center lg:justify-start">
                        <nuxt-link class="mt-4 h-10 px-8 flex justify-center items-center border border-black ml-font-bold-hover rounded-full transition-all ease-linear delay-75" to="/shop">
