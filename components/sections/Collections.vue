<template>
    <div>
        ...
        <div class="my-5 xl:my-10 px-5 xl:px-10">
            <div v-if="!loading && items.length==0" class="flex flex-wrap items-center bg-white p-5 border border-gray-300 rounded-lg">
            ...
            <div class="w-full lg:w-1/2 align-center mt-8 lg:mt-0 flex items-center justify-center lg:items-start lg:justify-start ">
                <div>
                    <h2 class="text-lg lg:text-xl font-bold text-center lg:text-right">{{ 'You don\'t have any collection' }}</h2>
                    <p class="text-sml mt-2 text-center lg:text-right">{{ 'Go to your store admin > products > collections >' }} <a href="//business.storeino.com/stores/collections" target="_blank" class="text-primary underline">{{ 'Add new' }}</a></p>
                    <div class="flex justify-center lg:justify-start">
                        <nuxt-link class="mt-4 h-10 px-8 flex justify-center items-center border border-black ml-font-bold-hover rounded-full transition-all ease-linear delay-75" to="/shop">
                            <span class="text-sml">{{'View theme demo'}}</span>
                        </nuxt-link>
                    </div>
                </div>
            </div>
