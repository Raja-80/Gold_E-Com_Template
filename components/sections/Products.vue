<template>
    <div class="px-12 lg:py-20 py-12">
        ...
        <div v-if="!loading && items.length > 0 && products.show_title" class="flex flex-col justify-center items-center lg:pb-24 pb-14 lg:px-10 px-0">
            <h1 class="text-black font-semibold text-center d lg:text-3xl md:text-2xl sm:text-xl text-xl">
        ...
        <div class="py-5">
            ...
            <carousel v-if=" !loading && items.length > 0" 
            ...
            <div v-if="!loading && items.length==0" class="px-5 my-5 xl:my-10 xl:px-10">
                <div class="flex flex-wrap items-center p-5 bg-white border border-gray-300 rounded-lg">
                    ...
                    <div class="flex items-center justify-center w-full mt-8 lg:w-1/2 align-center lg:mt-0 lg:items-start lg:justify-start ">
                        <div>
                            <h2 v-if="this.$store.state.language.code == 'AR'" class="text-lg font-bold lg:text-xl text-center-res">{{ 'ليس لديك أي منتحات' }}</h2>
                            <h2 v-else-if="this.$store.state.language.code == 'FR'" class="text-lg font-bold lg:text-xl text-center-res">{{ 'Vous n\'avez aucune product' }}</h2>
                            <h2 v-else class="text-lg font-bold lg:text-xl text-center-res">{{ 'You don\'t have any product' }}</h2>
                            <p v-if="this.$store.state.language.code == 'AR'" class="mt-2 text-sml text-center-res">{{ 'انتقل إلى مسؤول المتجر الخاص بك > المنتجات' }} <a href="https://business.storeino.com/stores/products" target="_blank" class="underline text-primary primary-hover">{{ 'اضف جديد' }}</a></p>
                            <p v-else-if="this.$store.state.language.code == 'FR'" class="mt-2 text-sml text-center-res">{{ 'Accédez à l\'administration de votre boutique > produits ' }} <a href="https://business.storeino.com/stores/products" target="_blank" class="underline text-primary primary-hover">{{ 'Ajouter nouveau' }}</a></p>
                            <p v-else class="mt-2 text-sml text-center-res">{{ 'Go to your store admin > products ' }} <a href="https://business.storeino.com/stores/products" target="_blank" class="underline text-primary primary-hover">{{ 'Add new' }}</a></p>
        ...
        <div v-if="!loading && items.length > 0 && products.active_show_more_btn" class="flex flex-col justify-center items-center">
...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            console.log('start async fetch function');
            let filter = { status: 'PUBLISH' };
            if (this.collections && this.collections.length > 0) filter['collections._id-in'] = this.collections.map(c => c._id);
